@extends('wizard')
@section('wizardtitle')
STEP 2: Qualifying Information 
@stop
@section('step') data-step-number="step2" id="imstep2" @stop
@section('wizardcontent')

	<h4 class="info-text"> This information is needed to calculate your NEW Federal Student Loan Re-Payment Options. The next few questions are based on your <b>last year's tax return filings</b></h4>
  ...
  <center>Are you <label><input type="radio" name="married" value="0" class="married_radio_status" {{ $MaritalStatus != 'Married' ? 'checked="checked"' : '' }} required> Single</label> or <label><input type="radio" name="married" value="1" class="married_radio_status" {{ $MaritalStatus == 'Married' ? 'checked="checked"' : '' }}  required> Married</label></center>
  <br/>
  ...
  <div class="show_married_0 form-group" style="display:none;">
  	...
  	<div class="col-sm-8">
		<div class="input-group">
			<span class="input-group-addon">$</span><input type="text" name="single_agi" id="single_agi" class="form-control" value="{{{ $Income_Yearly }}}"  required>
		</div>
	</div>
  </div>  
  ...
  <div class="show_married_1 form-group" style="display:none;">
	  <label class="col-sm-5 control-label" for="filestatus">Did you file jointly on your last tax return?</label>
 	  <div class="col-sm-7"><select class="form-control married_filed_jointly" name="married_filed_jointly"><option value="no" {{ $TaxFilingStatus == 'Married Filing Separately' ? 'selected="selected"' : '' }}>No</option><option value="yes" {{ $TaxFilingStatus == 'Married Filing Jointly' ? 'selected="selected"' : '' }}>Yes</option></select></div>
  </div>
  ...
  <div class="show_filed_jointly_yes form-group" style="display:none;">
  	<label class="col-sm-5 control-label" for="single_agi">Your combined Adjusted Gross Income (AGI)</label>
  	 <div class="col-sm-7">
  	   	<div class="input-group">
			<span class="input-group-addon">$</span><input type="text" name="married_agi" class="form-control" value="{{{ $CoIncome_Yearly }}}">
		</div>
	</div>
  </div>
  <div class="show_filed_jointly_no form-group" style="display:none;">
  	...
  	<div class="col-sm-8">
  		<div class="input-group">
			  <span class="input-group-addon">$</span><input type="text" name="single_agi2" class="form-control" value="{{{ $Income_Yearly }}}">
		</div>
	</div>
  </div>
  ...
  <div class="show_married_1" style="display:none">
	  <div class="form-group">
	    ...
	    <div class="col-sm-7"><input type="text" name="spouses_first_name" class="form-control" id="SpousesName_f" placeholder="Enter your Spouses First Name" value="{{{ $CoFirstName }}}" ></div>
	  </div>   
	  
	  <div class="form-group">
	    <label for="SpousesName_l" class="col-sm-5 control-label">Spouses Last Name</label>
	    <div class="col-sm-7"><input type="text" name="spouses_last_name" class="form-control" id="SpousesName_l" placeholder="Enter your Spouses Last Name"  value="{{{ $CoLastName }}}"></div>
	  </div>   
	  
	  <div class="form-group">
	    <label for="SpousesSSN" class="col-sm-5 control-label">Spouses SSN</label>
	    <div class="col-sm-7"><input type="text" name="spouses_ssn" class="form-control" id="SpousesSSN" placeholder="Enter your Spouses SSN"  value="{{{ $CoSSN }}}"></div>
	  </div>
	  
	  <div class="form-group">
	    <label for="SpousesDob" class="col-sm-5 control-label">Spouses Date of Birth</label>
	    <div class="col-sm-7"><input type="text" name="spouses_dob" class="form-control" id="SpousesDob" placeholder="Enter your Spouses Date of Birth"  value="{{{ $CoDOB }}}"></div>
	  </div> 
  </div>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-5"><input type="text" name="dependents" class="form-control" id="Dependents" placeholder="Number of dependents that you filed on your last tax return"  value="{{{ $FamilySize }}}"  required></div>
  </div> 
  ...
  <div class="form-group">
    ...
    <div class="col-sm-7"><select name="public_service" id="public_service_select" class="form-control"><option value="0" {{ $PublicService != 'Yes' ? 'selected="selected"' : '' }}>No</option><option value="1" {{ $PublicService == 'Yes' ? 'selected="selected"' : '' }}>Yes</option></select></div>
  </div> 
