  ...
  <!-- navigation -->
	...
	<div class="row">
			...
			@if (defined('MAIN_TEXT_TOP'))
					<h2 class="page-title" style="margin-top:14px;">{{ MAIN_TEXT_TOP }}</h2>
			@endif
    		</div>
    	</div>
    ...
    <div class="container-fluid">
        ...
        {{--// @BkroW Fixed the layout issue for video playback actions--}}
...
//                            @BKrow: Change this video id part above to change the video
            ...
            <div class="{{ Session::get('accepted_disclosure') ? 'col-sm-7 col-md-8 col-md-offset-4  col-lg-9 col-sm-offset-5  col-lg-offset-3' : 'col-sm-10 col-sm-offset-1' }}">
                ...
                {{-- @Bkrow: This is how you comment in Blade <3 --}}


                @include('snipplets.maintenance')


                @if (!Session::get('accepted_disclosure'))
                    @include('layouts.disclosure')
                @else

                    <div class="row">
                        <div class="col-sm-12">
                            @if (!isset($hide_returning_customer) || !$hide_returning_customer )
                                {{--// Mr. B // Fixed undefined variable for secured servers w/ error_reporting(0) disabled.--}}
                                <?php if (!$_GET['notice']): ?>
                                        <!-- JE	<table width="100%">
					<tr>
						...
						<td height="55px" width="200" align="right" bgcolor="#FFFF80">
							<input type="checkbox" id="returning_check_box" name="returning_check_box" {{ Session::get('ShowReturnPanel') ? 'checked="checked"' : '' }}>
                    ...
                    @if (!$hide_returning_customer)
                        <div class="returning_client_container">
                            <div id="returning_client" class="{{ Session::get('ShowReturnPanel') ? '' : 'shown' }}">
...
//	});
    	...
    	jQuery(document).bind('CheckIsDocumentOwner', function(){
			if (jQuery('#dontcheckagain').is(':checked')) return false;
				...
				jQuery.get('{{ Url::to('/IsDocumentOwner/') }}', {'csl': CurrentStepsLoaded}, function(result){
