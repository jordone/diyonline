...
@section('wizardtitle')
{{ Session::get('checkout_title') ? Session::get('checkout_title') :  "Checkout" }}
@stop

@section('step', 'data-step-number="step7"')
...
@section('wizardcontent')
    ...
    @include('snipplets.steps_header_text', ['step' => 'STEP7'])
			...
			<div class="panel-body">
			  	...
			  	<div class="row">
					...
					<script>
						...
						if (document.getElementById('consolidation_promisory_note').checked) { document.getElementById('Tprice').value = '{{ PRICE_CONSOLIDATION_NOTE }}'; }
						<?php endif; ?>
						<?php if (PRICE_RECERTIFICATION_APP > 0): ?>
						if (document.getElementById('recertification_app').checked) { document.getElementById('Tprice').value = '{{ PRICE_RECERTIFICATION_APP }}'; }
						<?php endif; ?>
						<?php if (PRICE_FOREBEARANCE_APP > 0): ?>
						if (document.getElementById('forebearance_app').checked) { document.getElementById('Tprice').value = '{{ PRICE_FOREBEARANCE_APP }}'; }
						<?php endif; ?>
					</script>
					<input type="text"  class="form-control"  name="tprice" id='Tprice' style="float:left" value="0.00" readonly="1">
					<!-- PRZ <input type="text"  class="form-control"  name="tprice" id='Tprice' style="float:left" value="{{ $client->TProperties->Payment1Amount }}" readonly="1"> -->
					<!-- PRZ <input type="text"  class="form-control"  name="tprice" id='Tprice' style="float:left" value="{{ number_format(PRICE_CONSOLIDATION_NOTE,2) }}" readonly="1"> -->
			...
			-->
				...
				<h3 class="page-title">Billing Information </h3>
					...
					<div class="form-group">
						...
						<div class="col-sm-9"><input class="form-control" name="Billing_first_name" value="{{ $client->TProperties->FirstName }}" placeholder="First Name" required></div>
					</div>
					
					<div class="form-group">
						<label class="control-label col-sm-3">Last Name</label>
						<div class="col-sm-9"><input type="text" class="form-control" name="Billing_last_name" value="{{ $client->TProperties->LastName }}"  placeholder="Last Name" required></div>
					</div>				
					<div class="form-group">
						<label class="control-label col-sm-3">Address</label>
						<div class="col-sm-9"><input type="text" class="form-control" name="Billing_address" placeholder="Address" value="{{ $client->TProperties->AddressLine1 }}" required></div>
					</div>				
					<div class="form-group">
						<label class="control-label col-sm-3">City</label>
						<div class="col-sm-9"><input type="text" class="form-control" name="Billing_city" placeholder="City" value="{{ $client->TProperties->City }}" required></div>
					</div>				
					<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-sm-9"><input type="text" class="form-control" name="Billing_state" placeholder="State" value="{{ $client->TProperties->State }}" required></div>
					</div>				
					<div class="form-group">
						<label class="control-label col-sm-3">Zip/Postal Code</label>
						<div class="col-sm-9"><input type="text" class="form-control" name="Billing_zipcode" placeholder="Zip Code" value="{{ $client->TProperties->ZipCode }}" required ></div>
					</div>
	
					<div class="form-group">
						<label class="control-label col-sm-3">Email</label>
						<div class="col-sm-9"><input type="email" class="form-control" name="Billing_email" placeholder="Email Address" value="{{ $client->TProperties->EmailAddress }}" required></div>
					</div>
